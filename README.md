...
**And the status of that claim, because it is the weakest one here.** The instruments were built against Hume's criteria and against what a clip pipeline can dose with a right answer behind it. The audience above was identified afterwards, as a positioning decision with no engine change — each flaw family maps onto a real failure mode of generated audio, and that mapping is an argument rather than a finding. **One person who makes music with an AI tool has now been asked, and said the premise does not hold for them** — that using Suno, sound quality is not something they worry about. That is a single respondent and it is the project's own owner, so it is weak evidence; it is also the only evidence that exists, and it points against the claim rather than for it. The two user findings this project does rest on came from listener interviews for a Columbia Business School engagement, and they are about listeners: that past listening predicts less than present and forming taste, and that almost nobody can describe their own taste in words. The second of those is the one this product serves least — it turns *damage* into language, not *preference* — and that gap is the open question, stated here rather than in a footnote.
