...
**And the status of that claim, because it is the weakest one here.** The instruments were built against Hume's criteria and against what a clip pipeline can dose with a right answer behind it. The audience above was identified afterwards, as a positioning decision with no engine change — each flaw family maps onto a real failure mode of generated audio, and that mapping is an argument rather than a finding. **No AI music producer has been interviewed.** The two user findings this project does rest on came from listener interviews for a Columbia Business School engagement, and they are about listeners: that past listening predicts less than present taste, and that almost nobody can describe their own taste in words. Both are served by what is built. Whether the people named above want it is the open question, and the honest place to say so is here rather than in a footnote.
