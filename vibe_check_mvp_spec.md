...
- **Integration choice — No-Code static Payment Link** (of Dodo's three: No-Code link / Inline-Overlay / SDK). Inline/Overlay is an **iframe/modal → rejected** (breaks in IG/TikTok in-app webviews; webview survival is mandatory). Checkout Sessions (SDK) also give a webview-safe redirect but add a create-API call (and the unverifiable `confirm:true` behaviour); the **static link is the fewest-failure-modes path** for our single fixed-price product — no create call at all.
- **Adapter contract** (`PaymentProvider`): `createCheckout` (builds the hosted-link redirect, no network), `orderRefParam`, `verify(orderRef) → {paid, token}` (the only API call). Implemented with `fetch` (no SDK). The `stripe` dependency is removed.
- **Flow (unchanged stateless model, no DB):** `/api/checkout` appends `redirect_url` (CLEAN, no query) + `metadata_profile=<token>` to the hosted Dodo Payment Link → **full-page redirect** to Dodo's hosted checkout (webview-survivable, not an iframe). Dodo returns to `…/premium/report?payment_id=…&status=…`. The report page **verifies the payment server-side** (`GET /payments/{id}`, status ∈ succeeded/completed/paid) — the live verify is the entitlement; the token comes from the verified `metadata.profile`. The return URL is kept query-free so the `payment_id` always parses (a paid customer must never fail to unlock); `?t=` is dev-unlock only. The client `status` param is never trusted.
...
- **Slice status:** Slice 1 = the variant behind a `voice` param defaulting to "classic" (default-off, shipped to git, no live change). Slice 2 = wire the transparent 50/50 `voice` arm via §10.A (sessionStorage-stable, URL-carried, analytics-tracked), **gated behind `NEXT_PUBLIC_VOICE_AB=1`** so deploying ≠ exposing — dormant until flipped, off in seconds if it bombs. Slice 3 = optional "Grounded in research" panel (Hume + Rentfrow citations, honest §9 caveat).
- **Rollout order:** push (dormant) → smoke-test live Haiku online voice with a real key → flip `NEXT_PUBLIC_VOICE_AB=1` → read the §6A-style share/conversion split by `voice` arm. Card image carries the voice; the share *link* drops it so new visitors get their own random arm (no inheritance — clean A/B). Link-unfurl previews stay archetype-based, so "online = more shareable" is tested mainly via downloaded-card shares.
...
## 27. Session log — fluid design system, transition polish & dev tooling (2026-06-23)

A long visual-design arc that converged the football and music surfaces onto ONE shared ambient "fluid" system, killed the dark↔bright navigation flash, and hardened the result page. All shipped to `main` (HEAD `649cace`). Worked under the per-task loop (north star → plan + alternative → smallest slice → prove with real output → red-team → confess).

### Decisions made
- **Football identity = bright "tournament drop" — NOT dark, NOT a frame.** Iterated hard under PM feedback: literal concentric-band frame (rejected as a heavy "box-in-a-box") → bright daytime stage → final **ambient fluid mesh**. Colour is *environment*, not chrome. The Design-Bar single-accent rule is intentionally overridden for this seasonal skin (PM call), but "one vivid accent per screen" is preserved via per-question rotation.
- **Music identity = dark/moody fluid that "reads you".** The ambient mesh **drifts toward the leading archetype's theme hue** in real time (reuses the existing `forming` hue-drift) — the "your taste reveals you" pitch made literal. Same primitive as football, opposite mood.
- **One shared primitive: `FluidField`** (`src/components/FluidField.tsx`) — palette-agnostic CSS radial-gradient mesh, slow GPU drift, **opacity cross-fade** when colours change (you can't transition `background-image`), **transparent base** so the page surface comes from the eased `--app-bg`. Football feeds host-hue colours on a light base; music feeds drift-hue colours on a dark base. The football skin stays scoped/deletable (`src/app/quiz/` + a `TOURNAMENT_SKIN` kill switch); `FluidField` is the only shared addition.
- **Colour logic (corrected with real theory):** **analogous harmony** (hue ±30) around the active hue — *not* near-complementary (reads as mud); the **whole field derives from the active hue** so it rotates wholesale per phase (Canada crimson → USA royal → Mexico emerald) / per drift — fixing "only the top third changed" (old bug: only the accent blob was phase-driven). **Silver/desaturated** for the neutral `static` archetype (its hue collides with midnight). **Vignette** for painterly depth. **Linear** progress bar = solid single tone; **multi-dimensional** forming bars = per-axis colours (logic-driven, not arbitrary).
- **Share cards now match the quizzes (funnel cohesion).** Football card → light fluid mesh + dark ink, with `readableAccent` darkening light nationality golds (BRA/ESP) for legibility on light; music card → dark mesh in the archetype theme hue. Music labelled rows + sigil preserved. Country **names removed** from the quiz header (read as misplaced); per-phase motifs kept.
- **Premium motifs, IP-safe (§3/§5/§13.D reaffirmed):** hand-rolled geometric **maple leaf** (curve path — a polygon attempt rendered as a starburst), **8-point starburst** (USA), **8-ray solar-ring** (Mexico). No flags, no eagle/Escudo (legally restricted in Mexico), no coats of arms, no "World Cup"/"FIFA"; hosts evoked by **colour + abstract glyphs** only.
- **"Flash-bang" transition fix = universal, no experimental APIs.** Body `--app-bg` eased via CSS `transition: background-color` (per-route luminance floor set by `RouteBackground`) + a content fade keyed by pathname + the `FluidField` cross-fade. **Native View Transitions rejected** — React 19.2 *stable* doesn't export `unstable_ViewTransition` (broke the build); the universal approach works in every in-app webview. **Framer Motion rejected** (bundle cost vs the webview constraint).
- **Result reading made crash-proof.** The SSR self-fetch to `baseUrl()/api/(music-)reading` could return an **HTML error page** → `res.json()` threw `Unexpected token '<'`. Fix: **compute the reading in-process** (dev always; prod only on a non-OK-JSON response) via `buildProfile`/`buildMusicProfile` + `narrate*` (which already have no-key/error fallbacks). The CDN-cached route remains the prod happy path.
- **Perf:** dropped the permanent `will-change: transform` on the mesh (pinned a compositor layer all session; the infinite transform auto-promotes anyway).

### Technical context discovered
- **Satori (`@vercel/og`) constraints:** no `backdrop-filter` (the "glass" UI concept was rejected for this + webview perf); `radial-gradient` + `hsla()` ARE supported. An explicit `backgroundColor: undefined` makes Satori call `.trim()` on undefined → **500** (caught + fixed by omitting the prop).
- **CSS gotcha:** `hsl()` colours can't take a hex-alpha suffix (`${hsl}1f` is invalid → silently dropped). Use `hsl(... / a)` / `hsla()`.
- **`baseUrl()` fragility hits local dev too:** the documented VERCEL deployment-protection 401-HTML risk also bites `npm run dev` (a server component fetching its own dev server mid-compile returns HTML). In-process compute sidesteps it; the dev self-fetch had no caching benefit anyway.
- **Verification tooling:** the preview screenshot wedges on the large 1080² card PNGs — card colours were verified by **canvas pixel-sampling** (base/ink/blob hexes), which is stronger proof for colour than a screenshot.
- **`.claude/` reload:** hooks and subagents are read at **session start**; mid-session additions need a restart (or `/hooks` / `/agents`) to go live.

### Dev tooling set up (local only, not committed)
- `/optimize` perf-review command → `.claude/commands/optimize.md`.
- `context-tracker.py` wired as `UserPromptSubmit` + `Stop` hooks in `.claude/settings.local.json` (personal context-usage tracker; it prints to stderr + exits 0, so the report isn't surfaced on success unless switched to JSON `systemMessage` on stdout).
- Three subagents in `.claude/agents/`: `implementation-agent`, `debugger`, `performance-optimizer`.

### Next steps / open
- **`static`-archetype silver** verified via the card path + code, not by landing on it live in the quiz (hard to force via taps).
- **Music card centre reads dark** (blobs sit in the four quadrants, none central) — optional soft central glow if more mid-card colour is wanted.
