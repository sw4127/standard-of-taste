...
| ~~`PAYMENTS_PROVIDER`, `DODO_PAYMENT_LINK`, `DODO_API_KEY`, `DODO_MODE`~~ | **unused since 2026-09-24**. There is no paid tier (D4 amendment); `/api/checkout` and `src/lib/payments/` were removed with the retired paid-tier code | safe to delete from the Vercel project; nothing reads them |
...
- ~~**Dodo account is personal** (mainland-China founder constraint, spec §24). Buyer path: their own MoR/Stripe behind the provider-agnostic adapter (`src/lib/payments/`).~~ *2026-09-24: moot. The adapter was removed; there is no paid tier (D4 amendment). The Dodo account, if it still exists, holds nothing this code uses.*
