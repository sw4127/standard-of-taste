...
| **the PRD** | A product requirements document for a reviewer at a larger company. Brief: `docs/task-prd.md` | **Parts 1, 2 and 3 of 4 done** — `docs/prd-1-use-cases.md`, the use-case inventory, 17 cases labelled 3 EVIDENCED / 14 ASSUMED, every one of 35 routes accounted for. Part 2 is `docs/prd-2-features.md` — sixteen features scored on craft and legibility, ranked under three weightings, rankings recomputed by test. Part 3 is `docs/prd-3-requirements.md` — functional requirements per instrument, each citing an exported symbol a test verifies, plus the non-goals assembled from the six refusals. **OPEN: part 4, wireframes** |
