...
| **T** — the prompt card | The terminal surface: three flaw families mapped to three prompt axes, assembled from templates, sitting above the threshold readout per RT-Z7 (b) | **done** 2026-09-16, six slices. The band count is MEASURED (`docs/analytics/e21-card-bands.txt`): pitch 2, timing 2, compression **1** — that ladder supports no fineness claim at all. `prompt-card.ts` computes the structure, `content/card/` assembles the words, `PromptCardPanel` renders above the threshold figure with the expert view untouched. **What is NOT done: the copy has never been through a writing pass**, and the card's commonest output on the thinner ladders is "not enough to tell" — across listeners drawn over a ladder's whole range, timing returns a point estimate in 30% of sittings and one compression source in 16% |
