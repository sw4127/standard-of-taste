...
export default function Page() {
  return (
    <Explainer page={page} kicker="MACHINE 01 · THE FLAGSHIP">
      ...
      <p>
        The design is a within-subject experiment, about {BIAS_SESSION_MINUTES} minutes long. You hear{" "}
        {numberWord(BIAS_CLIP_COUNT)} short clips and rate each one <strong>blind</strong> — no artist, no
        context, just sound. Then you hear the same {numberWord(BIAS_CLIP_COUNT)} clips again with names
        and reputations attached, and rate them again. Your
      ...
      <p>
        ...
        <strong>
          {numberWord(BIAS_SWAPPED_COUNT)} of the {numberWord(BIAS_LABELLED_COUNT)} labels are deliberately
          false.
        </strong>{" "}A modest work arrives
      ...
      <p>
        {numberWordLeading(BIAS_CONTROL_COUNT)} of the {numberWord(BIAS_CLIP_COUNT)} clips are{" "}
        <strong>controls</strong>: they carry no label in either pass.
