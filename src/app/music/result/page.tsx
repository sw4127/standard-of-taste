...
import { SITE_NAME, SITE_WORDMARK } from "@/content/site";
...
export async function generateMetadata({ searchParams }: { searchParams: SearchParams }): Promise<Metadata> {
  ...
  if (missingWeighted(musicQuiz, answers).length > 0) return { title: SITE_NAME };
  ...
  return {
    title: `My vibe is ${profile.archetype.label} — ${SITE_NAME}`,
...
export default async function MusicResultPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-md flex-col px-6 py-8">
      ...
      {/* THE LATELY LINE, KEPT; THE PAYWALL, NOT (2026-09-23, RT-4 (c)). This box
          was the paid read's doorway. The paywall stays retired, so the link and
          its "Get the full read" button are gone — but the line itself is the
          state lane of the owner's thesis (spec §9, P4: recent taste reads
          current state), the part of the snack that speaks to what someone has
          been going through lately. It stays, as a reading, not an ask. */}
      <div
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-white/10">
        ...
        <img src={storyUrl} alt="Your shareable snack card" className="w-full" />
      </div>
      <div className="mt-4 flex flex-wrap justify-center gap-3">
        <ShareButton url={shareUrl} text={shareText} label="Share my card" event="share_native" accent={accent} primary />
        <DownloadButton url={storyUrl} label="Download (Story)" filename="standard-of-taste-story.png" />
        <DownloadButton url={squareUrl} label="Download (Square)" filename="standard-of-taste-square.png" />
      </div>
