...
export const worldCupRoster: CentroidSet = {
  ...
  centroids: [
    ...
    { id: "yamal", label: "Lamine Yamal",
      // Maverick lean: flair-primary with electric INTENSITY (only 2 axes high —
      // stays on the reachable manifold). Splits the flair corner with cherki
      // (pure flair, calmer), which also relieves cherki's over-concentration.
      vector: { intensity: 0.72, flair: 0.92, workrate: 0.42, composure: 0.55, teamplay: 0.5 },
    ...
    { id: "bellingham", label: "Jude Bellingham",
      // Showman/Gladiator lean: box-to-box clutch power — intensity + workrate
      // primary (2 high axes, on the manifold), with more flair/presence than
      // valverde's pure low-flair engine (revives a never-matched player).
      vector: { intensity: 0.85, flair: 0.62, workrate: 0.84, composure: 0.6, teamplay: 0.6 },
    ...
    { id: "debruyne", label: "Kevin De Bruyne",
      // Maestro/Architect lean: chess-master vision — flair + teamplay primary
      // (2 high axes, on the manifold), the passing geometry. Distinct from
      // maza's low teamplay and modric's lower flair (revives a dead player).
      vector: { intensity: 0.58, flair: 0.86, workrate: 0.62, composure: 0.6, teamplay: 0.86 },
    ...
    { id: "rodrygo", label: "Rodrygo",
      // Clutch/understated: composure + intensity primary (vacates the crowded
      // flair zone where nicopaz dominated it — keeps it reachable).
      vector: { intensity: 0.66, flair: 0.66, workrate: 0.6, composure: 0.86, teamplay: 0.6 },
      tags: ["clutch", "smooth", "big-game", "understated"] },
    { id: "raphinha", label: "Raphinha",
      // Fiery winger: intensity + flair, LOW composure (vacates bellingham's
      // intensity+workrate zone that was dominating it — keeps it reachable).
      vector: { intensity: 0.85, flair: 0.82, workrate: 0.66, composure: 0.48, teamplay: 0.55 },
    ...
    { id: "wirtz", label: "Florian Wirtz",
      // Maestro/Maverick lean: elusive tight-space wizardry — flair + composure
      // primary (2 high axes, on the manifold), the silky creator. Distinct from
      // cherki's volatility and nicopaz/mora's lower flair (revives a dead player).
      vector: { intensity: 0.5, flair: 0.9, workrate: 0.52, composure: 0.8, teamplay: 0.6 },
    ...
    { id: "maza", label: "Ibrahim Maza",
      // Press-resistant creator: flair + teamplay primary (vacates wirtz's
      // flair+composure zone that was dominating it — keeps it reachable).
      vector: { intensity: 0.6, flair: 0.9, workrate: 0.56, composure: 0.6, teamplay: 0.8 },
