...
p();
/*
 * QUOTED FROM THE BLUEPRINT OF RECORD, NEVER RETYPED (blueprint Part 7, BA-1).
 * This section described "a taste gym" that "does not predict personality, mood,
 * or psychological state, and it never has" until 2026-09-23, when the reading
 * became the product (BA-6) and the second half stopped being true of it.
...
for (const id of ["BP-INSIGHT", "BP-UNMET", "BP-BRIDGE"]) {
  if (!BLUEPRINT[id]) throw new Error("export-commission: " + id + " is missing from docs/blueprint.md");
}
p(
  "It is a **reading of a listener's recent taste**. The insight it rests on, quoted from " +
    "`docs/blueprint.md` (BP-INSIGHT):",
);
p();
p("> " + BLUEPRINT["BP-INSIGHT"]);
p();
p("What nothing else offers (BP-UNMET):");
p();
p("> " + BLUEPRINT["BP-UNMET"]);
p();
p(
  "Each line of the reading names a pattern in a month of plays, shows the plays behind it, and " +
    "offers two things it might mean as questions, plus \"neither\". The reader keeps or rejects " +
    "lines, and what survives becomes a prompt for a music generator. It runs on **three " +
    "illustrative listeners** whose plays are generated and labelled as such, and **no sentence on " +
    "the site is generated by a language model when a visitor arrives** (BA-10): everything you are asked to edit is a " +
    "template.",
);
p();
p("The instruments are the **hearing section**, reached from the prompt (BP-BRIDGE):");
p();
p("> " + BLUEPRINT["BP-BRIDGE"]);
p();
p("Four hearing tests are live:");
p();
p(
 ...
 */
p(
  "**D1 has TWO exceptions, each one surface wide.** The **prompt card** (2026-09-16) — the block " +
    "that turns a measured threshold into words for a generator — and **the reading** (2026-09-23), " +
    "may speak to the reader about what something might mean **for them**. Every other surface is " +
    "unchanged: the instrument readouts, the reading room, `/method` and the Company view stay on " +
    "what the listener DID. The register on both is **offer, do not assert**, and on the reading it " +
    "is stricter still (BA-3): a line names a pattern and never a feeling, and the offered " +
    "readings are questions. \"You chose the take with the slower decay every time\" is the " +
    "target; \"you have unresolved loss\" is refused; and no sentence anywhere asserts anything " +
    "about trauma, abuse or mental health. Each surface says on itself that it is doing this. Full " +
    "text: `CLAUDE.md`, \"D1 amendment\" and \"D1 amendment, third surface\".",
);
p();
p(
